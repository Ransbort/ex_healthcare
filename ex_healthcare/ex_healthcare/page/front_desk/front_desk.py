...
def create_consultation(
	...
	if float(consultation_fee or 0) > 0:
		invoice_name = _create_paid_consultation_invoice(patient, consultation_fee, appointment.name)
		appointment.db_set("consultation_invoice", invoice_name)
		appointment.db_set("queue_status", "Paid - Awaiting Vitals")
	else:
		appointment.db_set("queue_status", "Payment Pending")
...
def _create_paid_consultation_invoice(patient, amount, appointment_name):
	"""Create a POS-style, already-settled Sales Invoice for the consultation fee.

	ASSUMPTION: assumes an Item named 'Consultation' exists and is billable.
	If your site uses a different item/service code for consultation fees,
	change CONSULTATION_ITEM_CODE below or pass it through as a parameter.
	"""
	...
	mode_of_payment = frappe.db.get_single_value("Healthcare Settings", "default_mode_of_payment") or "Cash"
...
# =============================================
# QUEUE (shared by Front Desk / Nurse / Doctor views)
# =============================================
...
def get_queue(date=None, queue_status=None):
	...
	rows = frappe.get_all(
		...
		fields=[
			...
			"department", "appointment_type", "appointment_time", "queue_status",
			"consultation_invoice", "checked_in_at",
			"vitals_temperature", "vitals_blood_pressure", "vitals_pulse",
			"vitals_weight", "vitals_height", "vitals_notes",
		],
...
@frappe.whitelist()
def save_vitals(appointment, temperature=None, blood_pressure=None, pulse=None,
				 weight=None, height=None, notes=None):
	doc_updates = {
	...
	for field, value in doc_updates.items():
		frappe.db.set_value("Patient Appointment", appointment, field, value)
	return {"status": "Success"}
...
def start_consultation(appointment):
	"""Mark the appointment as In Consultation and return details the client
	can use to open/prefill a new Patient Encounter."""
...
def on_patient_encounter_submit(doc, method=None):
	"""Doc event hook (wire up in hooks.py) — once the doctor submits the
	Patient Encounter, automatically close out the queue entry."""
